  ...
  return (
    <div className="h-full flex flex-col">
      <div className="flex justify-end mb-4">
        <BlobProvider document={<EbookDocument />}>
          {({ url, loading }) => (
            url ? (
              <a
                href={url}
              ...
              <button
                disabled
                className="inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-blue-400 cursor-not-allowed"
              >
                <Download className="w-4 h-4 mr-2" />
      ...
      <div className="flex-1 bg-gray-50 rounded-lg border-2 border-dashed border-gray-300 flex items-center justify-center">
        <p className="text-gray-500">PDF preview is temporarily disabled</p>
      </div>
    </div>
  );
}