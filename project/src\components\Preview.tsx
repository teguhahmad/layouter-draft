    ...
    try {
      ...
      chapters.forEach((chapter, index) => {
        ...
        chapter.subChapters.forEach((subChapter) => {
          ...
          const parsedSubContent = parseMarkdown(subChapter.content);
          const subContentDiv = document.createElement('div');
          subContentDiv.innerHTML = parsedSubContent;
          const subTextContent = subContentDiv.textContent || '';

          const subLines = doc.splitTextToSize(subTextContent, contentWidth);
              ...
              {chapter.subChapters.map((subChapter) => (
                <div key={subChapter.id} className="mt-8">
                  ...
                  <div style={{
                    ...
                    dangerouslySetInnerHTML={{ __html: parseMarkdown(subChapter.content) }}
                  />
                </div>
              ))}
            </div>
